...
public class ScreenScraper {
    ...
    public GamesTableOfLeague getGamesTableOfCompetition(String competitionYears) {//name of method!! i want to bring table of all (round) games
        ...
        for(Element row : rows) {
        	if(kindOfElement(row).kind == KindOfRow.Round) {
        		...
        		String date ="";
    			String time="";
        		while((j<rows.size()) && (kindOfElement(rows.get(j)).kind !=KindOfRow.Round)){
        			Elements tds = rows.get(j).children();
        			Game game ;
        			
        			if(tds.size() >=6) {
        				if(kindOfElement(tds.get(0)).kind == KindOfRow.Date) {
    ...
    public KindOfElement kindOfElement(Element element) {
        KindOfElement result = new KindOfElement();
        result.kind = KindOfRow.Error;
    ...
    RankingTable getLastTable(String competitionName, String years) {
    ...
    RankingTable getTableByRound(String competition, String years, String round) {
