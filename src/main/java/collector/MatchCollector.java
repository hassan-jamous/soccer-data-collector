...
public class MatchCollector {
    ...
    public MatchDetails getMatchDetails(String competitionName, String competitionYears, String firstClub, String secondClub) {
        ...
        result.attendance = getMatchAttrndance(allTablesInPage);
        result.staduim = getMatchStaduim(allTablesInPage);
        return result;
    }

    private String getMatchStaduim(Elements allTablesInPage){
    ...
    private String getMatchAttrndance(Elements allTablesInPage){
        
    	Elements elementAttrndance = allTablesInPage.select("tr:has(td:has(img[title=Attendance]))");
        if (elementAttrndance != null) {
            return elementAttrndance.text();
        }
    ...
    private ClubInMatch getClubInformation(Elements allTablesInPage, int teamNumber, String clubName) {
        ...
        result.players = getClubPlayersInMatch1111(elementClubsInfo.get(teamNumber));
    ...
    //todo
    //no copy paste 
    //but there is a problem with this style of (if , else) when i == indexOfRowWithSubstitutesTitle
    private ArrayList<PlayerAtMatch> getClubPlayersInMatch1111(Element table){
