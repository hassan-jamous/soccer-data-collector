...
public class RoundCollector {
	 ...
	 * @param stringToAddToURL if normal round this will be empty string or equals  to /slug/final or 
	 * /slug/qualification-round-2 according to round's kind
	 * French league 2017/2018 
	 * from this url  
	 * https://api.sofascore.com/api/v1/unique-tournament/Competition's ID/season/Season's ID/events/round/ Round Number
	...
	public RoundGamesID getGamesIdInRound(String competitionName, String competitionYears , String round , String stringToAddToURL) {
		
		String gsonString = httpUtil.sendGetHttpRequest(String.format(API_SOFA_SCORE_ROUND_URL,leagueId.getLeagueID(competitionName), seasonId.getSeasonID(competitionName , competitionYears),round,stringToAddToURL));
	...
	public void writeRound(String competitionName, String competitionYears , String round , String stringToAddToURL) {
		...
		if((gamesIdInRound == null) ||(gamesIdInRound.events == null) ||(gamesIdInRound.events.isEmpty())) {
			csvDealer.write(Sites.SofaScore_Com, competitionName, "no information about season "+competitionYears +"in round "+round, true, FileTypes.NoInformation);
		}
		else {
			for(int i =0 ; i < gamesIdInRound.events.size(); i++) {
				...
				else if(gameBasicInfromation.event.status.type.equals("inprogress")) {}//the game is being played
				else if(gameBasicInfromation.event.status.description.equals("Not started") && gameBasicInfromation.event.status.type.equals("notstarted")) {//not started
					if((i==0)&&(Integer.valueOf(round)==1)) {//first game to write the header
						csvDealer.write(Sites.SofaScore_Com, competitionName, csvGetterString.getHeaderStringForCSV(gameBasicInfromation) + csvGetterString.getHeaderStringForCSV(gameStatistic), true, FileTypes.Statistics);
					}
